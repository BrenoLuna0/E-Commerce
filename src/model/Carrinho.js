...
class Carrinho {
  static async adicionarNoCarrinho(userId, productId, qtd, filial) {
    ...
    return new Promise(async function (resolve) {
      conexao.execute(sql, [], { autoCommit: true }, function (err) {
        if (err) {
          console.log(
            "Erro no oracle para adicionar produto no Carrinho 001: " +
              err.message
          );
          resolve(false);
        } else {
  ...
  static async removerDoCarrinho(userId, productId, filial) {
    ...
    return new Promise(async function (resolve) {
      conexao.execute(sql, [], { autoCommit: true }, function (err) {
        if (err) {
          console.log(
            "Erro no Oracle ao deletar produto do carrinho 002: " + err.message
          );
          resolve(false);
        } else {
          resolve(true);
  ...
  static async getProdutos(userId, filial) {
    ...
    return new Promise(async function (resolve) {
      conexao.execute(sql, [], { autoCommit: true }, async function (
        err,
        result
      ) {
        if (err) {
          console.log(
            "Erro no Oracle ao requisitar produtos do carrinho 005: " +
              err.message
          );
          resolve({
            erro: true,
    ...
    const produtosdetalhes = await produtos.map(function (produto) {
      ...
      return new Promise(async function (resolve) {
        conexao.execute(sql, [], { autoCommit: true }, function (err, result) {
          if (err) {
            console.log(
              "Erro no Oracle ao requisitar produtos do carrinho 006: " +
                err.message
            );
            resolve([]);
          } else {
    ...
    const produtosImagem = resultado.map(function (produto) {
      ...
      return new Promise(async function (resolve) {
        await conexao.execute(sql, [], { autoCommit: true }, function (
          ...
          if (err) {
            console.log(
              "Erro no Oracle ao requisitar produtos do carrinho 007: " +
                err.message
            );
            resolve([]);
          } else {
            if (typeof result.rows[0] === "undefined") {
              ...
              resolve({
                ...
                qtd: produto.qtd,
                subtotal: formatarMoeda(produto.qtd * produto.preco),
                path: "http://187.84.80.162:8082/imagens/" + result.rows[0][0],
                qtd_estoque: produto.qtd_estoque,
              });
            }
          }

    ...
    .then((response) => {
      return response.map((produto) => {
        return {
          ...
          preco: produto.PROD_PRECO_VENDA,
          categoria: produto.SUB_GRP_DESCRICAO,
          qtd: produto.PROD_QTD_ATUAL,
        };
      });
    })
    .catch((err) => {
...
async function getNineProdutos(filial) {
  return await knex
    .raw(
      "SELECT * FROM (SELECT DISTINCT P.PROD_CODIGO, P.PROD_DESCRICAO, p.PROD_PRECO_VENDA, s.sub_grp_descricao, P.PROD_QTD_ATUAL FROM SIAC_TS.VW_PRODUTO P , siac_ts.vw_subgrupo S WHERE p.sub_grp_codigo = s.sub_grp_codigo AND P.FIL_CODIGO = ? AND P.PROD_ATIVO = 'S' AND p.PROD_PRECO_VENDA > 0 ORDER BY dbms_random.value ) where ROWNUM <= 9",
      [filial]
    )
    .then((response) => {
      return response.map((produto) => {
  ...
  AND P.PROD_CODIGO = ?`,
    ...
    .then((response) => {
      if (response.length === 0) {
        ...
        const produtoDados = response[0];
        const produto = {
          codigo: produtoDados.PROD_CODIGO,
          nome: produtoDados.PROD_DESCRICAO,
          preco: produtoDados.PROD_PRECO_VENDA,
          categoria: produtoDados.SUB_GRP_DESCRICAO,
          qtd: produtoDados.PROD_QTD_ATUAL,
        };
        return [produto];
      }
    })
...
async function getProdutosImagem(produtos) {
  const produtosArray = produtos.map(async function (produto) {
    return await knex
      .raw(
        "SELECT prod_imag_descricao, prod_imag_nome FROM SIAC_TS.vw_produto_imagem WHERE prod_codigo = ?",
        [produto.codigo]
      )
      .then((response) => {
        if (response.length === 0) {
          ...
          const images = response.map(function (imagem) {
            return "http://187.84.80.162:81/imagens/" + imagem.PROD_IMAG_NOME;
          });
          ...
          return new Produto(
            ...
            formatarMoeda(produto.preco),
            produto.categoria,
            response[0].PROD_IMAG_DESCRICAO || "Produto sem Descrição",
            images,
            produto.qtd,
            "picZoomer"
          );

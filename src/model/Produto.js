...
class Produto {
    constructor(PROD_CODIGO, PROD_DESCRICAO, PROD_PRECO, PROD_CATEGORIA, PROD_IMAG_DESCRICAO, PROD_IMAG_PATH, PROD_QTD_ATUAL, ZOOM) {
            ...
            if (err) {
                ...
                if (typeof (result.rows[0]) === 'undefined') {
                    ...
                    const objectArray = result.rows.map(function (produto) {
                        return { codigo: produto[0], nome: produto[1], preco: produto[2], categoria: produto[3], qtd: produto[4] };
                    });
        ...
        await conexao.execute(sql, [], { autoCommit: true }, function (err, result) {
            if (err) {
                ...
                if (typeof (result.rows[0]) === 'undefined') {
                    ...
                    const produto = { codigo: produtoDados[0], nome: produtoDados[1], preco: produtoDados[2], categoria: produtoDados[3], qtd: produtoDados[4] }
                    resolve([produto]);
                }
            }
        });

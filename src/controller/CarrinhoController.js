...
module.exports = {
  ...
  async showProducts(req, res) {
    ...
    //...Caso dê tudo certo, serão feitos alguns procedimentos:
      ...
      const produtosFinais = await Carrinho.getProdutosDetalhe(
        produtosCarrinho,
        req.session.filial
      );
      ...
      const produtosRelacionados = await Produto.produtosRelacionados(
  ...
  async adicionarAoCarrinho(req, res) {
     ...
     */
    const result = await Carrinho.adicionarNoCarrinho(
      req.user.id,
      req.body.produto,
      req.body.qtd,
      req.session.filial
  ...
  async removerDoCarrinho(req, res) {
     ...
     */
    const result = await Carrinho.removerDoCarrinho(
      req.user.id,
      req.body.produto,
      req.session.filial
    );
  ...
  async atualizarCarrinho(req, res) {
     ...
     * Estes respectivos valores (-true- e -false-) são retornados para a view
     */
    const result = await Carrinho.atualizarQuantidade(
      req.user.id,
      req.body.produto,
      req.body.qtd,
      req.session.filial
  ...
  async limparCarrinho(req, res) {
     ...
     * Esta função retornará -true- em caso de sucesso e -false- em caso de falha. Este valor está guardado dentro da variável <result>
     */
    const result = await Carrinho.limparCarrinho(
      req.user.id,
      req.session.filial
    );
    ...
    //Em caso de falha, é renderizada a página de erro, porém a compra foi concluída com sucesso. Só o carrinho que não foi esvaziado
      ...
      res.render("errors/manipuladorErro", {
        err: {
          ...
          msg:
            "Erro no final do processo da compra. Verifique se esta compra está disponível em seu histórico. Caso não esteja, entre em contato conosco",
          cod: "003",
        },
        cartTotal: "",
        filial: req.session.filialName,
